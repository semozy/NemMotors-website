          ...
          <section aria-label="Voertuig en prijs" className="py-1">
            <div className="mb-4 flex items-center justify-between gap-3">
              ...
              <div className="flex items-center gap-2">
                <label className="cursor-pointer"><input type="checkbox" className="peer sr-only" aria-label={`${name} selecteren voor vergelijking`} /><span className="flex min-h-9 items-center gap-1.5 rounded border border-neutral-200 px-2.5 text-[10px] font-semibold text-neutral-500 peer-checked:border-yellow-500 peer-checked:bg-yellow-50 peer-focus-visible:ring-2 peer-focus-visible:ring-yellow-500"><GitCompareArrows className="h-3.5 w-3.5" aria-hidden="true" />Vergelijk</span></label>
                <label className="cursor-pointer"><input type="checkbox" className="peer sr-only" aria-label={`${name} markeren als favoriet`} /><span className="flex h-9 w-9 items-center justify-center rounded border border-neutral-200 text-neutral-500 peer-checked:border-yellow-500 peer-checked:bg-yellow-50 peer-checked:text-yellow-700 peer-checked:[&>svg]:fill-current peer-focus-visible:ring-2 peer-focus-visible:ring-yellow-500"><Heart className="h-4 w-4" aria-hidden="true" /></span></label>
              </div>
            </div>
            ...
            <div className="grid gap-2.5">
              <button type="button" disabled className="flex min-h-12 cursor-not-allowed items-center justify-center gap-2 rounded bg-[#ffc20e] px-4 text-xs font-bold text-neutral-950"><CalendarDays className="h-4 w-4" aria-hidden="true" />Maak een proefrit</button>
        ...
        <section aria-labelledby="car-specifications" className="mt-10 border-t border-neutral-200 pt-7 sm:mt-12">
          ...
          {(sections.length > 0 || car.description || car.options?.length > 0) && (
            <details className="group mt-5">
              <summary className="inline-flex min-h-10 cursor-pointer list-none items-center gap-3 rounded border border-neutral-300 px-4 text-xs font-semibold hover:border-yellow-500 focus-visible:outline-yellow-500 [&::-webkit-details-marker]:hidden">
              ...
              <div className="mt-7 space-y-8">
                {sections.map(([title, rows]) => (
                  <section key={title} aria-label={title}>
                    <h3 className="border-l-4 border-[#ffc20e] pl-3 text-lg font-bold">{title}</h3>
                ...
                {car.options?.length > 0 && (
                  <section aria-label="Uitrusting">
                    <h3 className="border-l-4 border-[#ffc20e] pl-3 text-lg font-bold">Uitrusting</h3>
                    <ul className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                      {car.options.map((option, index) => <li key={`${option}-${index}`} className="flex items-start gap-3 rounded bg-neutral-50 px-4 py-3 text-sm text-neutral-700"><span aria-hidden="true" className="mt-1.5 h-1.5 w-1.5 shrink-0 rounded-full bg-yellow-500" />{option}</li>)}
                    </ul>
                  </section>
                )}
                {car.description && (
                  <section aria-label="Beschrijving">
                    <h3 className="border-l-4 border-[#ffc20e] pl-3 text-lg font-bold">Beschrijving</h3>
