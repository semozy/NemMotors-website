  ...
  return (
    <section aria-label={`Foto's van ${name}`}>
      <div className="relative flex aspect-[4/3] items-center justify-center overflow-hidden rounded-md border border-neutral-200 bg-[#e9eae7]" onKeyDown={(event) => {
        ...
        <span className="absolute left-4 top-4 rounded-sm bg-neutral-950 px-3 py-2 text-[9px] font-black uppercase tracking-[0.2em] text-white">NEM Motors</span>
        {count > 1 && <><button type="button" onClick={() => change(-1)} aria-label="Vorige foto" className="absolute left-4 top-1/2 flex h-11 w-11 -translate-y-1/2 items-center justify-center rounded-full bg-white/95 text-neutral-950 shadow-sm hover:bg-neutral-200"><ChevronLeft className="h-5 w-5" /></button><button type="button" onClick={() => change(1)} aria-label="Volgende foto" className="absolute right-4 top-1/2 flex h-11 w-11 -translate-y-1/2 items-center justify-center rounded-full bg-white/95 text-neutral-950 shadow-sm hover:bg-neutral-200"><ChevronRight className="h-5 w-5" /></button></>}
